...
function shouldSendStorageLowPhaseNotification(source, phase, now, lastNotifiedAtByPhase, intervalMs) {
	const lastNotifiedAt = lastNotifiedAtByPhase && lastNotifiedAtByPhase[phase] || 0;

	return shouldSendStorageLowNotification(source, now, lastNotifiedAt, intervalMs);
}

function sendStorageLowPhaseNotification(sendNotification, payload, phase, notifiedAt, lastNotifiedAtByPhase) {
	lastNotifiedAtByPhase[phase] = notifiedAt;

	return sendNotification(payload).then(result => {
		if (result && result.skipped && lastNotifiedAtByPhase[phase] === notifiedAt) {
			lastNotifiedAtByPhase[phase] = 0;
		}
...
function createStorageLowNotification(details) {
	const payload = {
		...
		title: details.severity === 'warning' ? '[Chinachu] WARNING: Storage Low Space!' : '[Chinachu] ALERT: Storage Low Space!',
	...
	if (details.phase) {
		payload.metadata.phase = details.phase;
	}
...
module.exports = {
	...
	sendStorageLowPhaseNotification: sendStorageLowPhaseNotification,
	shouldSendStorageLowNotification: shouldSendStorageLowNotification,
	shouldSendStorageLowPhaseNotification: shouldSendStorageLowPhaseNotification
};
